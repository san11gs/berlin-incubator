...
--%><%
  ...
  as3Imports.add("mx.events.PropertyChangeEvent");
  if (jClass.properties.find {Collection.class.isAssignableFrom(it.type)}) {
    as3Imports.add("mx.collections.ArrayCollection");
    as3Imports.add("mx.events.CollectionEvent");
  }
...
private var _${jProperty.name}:<% if (jProperty.isEnum()) { %>String;<% } else { %>${jProperty.as3Type.name};<% }
}
...
<%
  for (collectionProperty in jClass.properties.findAll {Collection.class.isAssignableFrom(it.type)}) {
%>
_${collectionProperty.name} = new ArrayCollection();
<%
  }
%>
}
...
public function set ${jProperty.name}(value:<% if (jProperty.isEnum()) { %>String<% } else { %>${jProperty.as3Type.name}<% } %>):void {
            if(_${jProperty.name} != value){
              var oldValue:<% if (jProperty.isEnum()) { %>String<% } else { %>${jProperty.as3Type.name}<% } %> = _${jProperty.name};
              _${jProperty.name} = value;
              dispatchEvent(PropertyChangeEvent.createUpdateEvent(this, "${jProperty.name}", oldValue, value));
            }
        }<%
    }
    if (jProperty.readable) { %>
public function get ${jProperty.name}():<% if (Enum.class.isAssignableFrom(jProperty.type)) { %>String<% } else { %>${jProperty.as3Type.name}<% } %> {
...
<%
    for (collectionProperty in jClass.properties.findAll {Collection.class.isAssignableFrom(it.type)}) {
%>
_${collectionProperty.name}.addEventListener(CollectionEvent.COLLECTION_CHANGE, listener);
<%
    }
%>
}
...
_${jProperty.name} = input.readObject() as String;<%
  }
  else { %>
